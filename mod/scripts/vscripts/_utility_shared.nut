...
{
	// modified in _base_gametype_mp.gnut, also make use of player entity here
	#if SERVER && MP
		if ( GetForcedDeathCamLength( player ) > 0 && !ignoreForced )
			return GetForcedDeathCamLength( player )
	#endif
