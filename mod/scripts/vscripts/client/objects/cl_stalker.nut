...
// removing all these hardcode, gibs now depending on their model!
#if SP
...
// ALT stalker in SP: red variant
const asset STALKER_MODEL_ALT = $"models/robots/stalker/robot_stalker_red.mdl"
const asset STALKER_GIB_LEG_L_ALT = $"models/robots/stalker/robot_stalker_l_leg_red_gib.mdl"
const asset STALKER_GIB_LEG_R_ALT = $"models/robots/stalker/robot_stalker_r_leg_red_gib.mdl"
const asset STALKER_GIB_ARM_L_ALT = $"models/robots/stalker/robot_stalker_l_arm_red_gib.mdl"
const asset STALKER_GIB_ARM_R_ALT = $"models/robots/stalker/robot_stalker_r_arm_red_gib.mdl"
#elseif MP
...
// ALT stalker in MP: white variant
const asset STALKER_MODEL_ALT = $"models/robots/stalker/robot_stalker.mdl"
const asset STALKER_GIB_LEG_L_ALT = $"models/robots/stalker/robot_stalker_l_leg_gib.mdl"
const asset STALKER_GIB_LEG_R_ALT = $"models/robots/stalker/robot_stalker_r_leg_gib.mdl"
const asset STALKER_GIB_ARM_L_ALT = $"models/robots/stalker/robot_stalker_l_arm_gib.mdl"
const asset STALKER_GIB_ARM_R_ALT = $"models/robots/stalker/robot_stalker_r_arm_gib.mdl"
#endif
...
void function ClStalker_Init()
{
	...
	Cl_StalkerModel_Init( $"models/robots/stalker/robot_stalker.mdl" ) // white stalker
	Cl_StalkerModel_Init( $"models/robots/stalker/robot_stalker_red.mdl" ) // red stalker
}
...
void function CreateCallback_Stalker( entity npc )
{
	...
	// that means first npc never receive model effects, and npcs with different model can't be recognized
	// maybe to reduce load till this kind of npc's first time spawn?
	// I'd like to init them manually though, because we also want white stalkers to appear in MP
	/*
...
// modified function for handling stalkers
void function Cl_StalkerModel_Init( asset model_name )
{
	// default gibs
	asset leftLegGib = STALKER_GIB_LEG_L
	asset rightLegGib = STALKER_GIB_LEG_R
	asset leftArmGib = STALKER_GIB_ARM_L
	asset rightArmGib = STALKER_GIB_ARM_R
	
	// ALT stalker model gibs
	if ( model_name == STALKER_MODEL_ALT )
	{
		leftLegGib = STALKER_GIB_LEG_L_ALT
		rightLegGib = STALKER_GIB_LEG_R_ALT
		leftArmGib = STALKER_GIB_ARM_L_ALT
		rightArmGib = STALKER_GIB_ARM_R_ALT
	}

	//------------------------------
	//DAMAGE FX - REGULAR STALKERS
	//------------------------------
	ModelFX_BeginData( "stalkerHealth", model_name, "all", true )
		//ModelFX_AddTagHealthFX( 0.75, "chest", $"P_spectre_dmg_smk", false )
		ModelFX_AddTagHealthFX( 0.5, "chest", $"P_spectre_dmg_elec", false )
		//ModelFX_AddTagHealthFX( 0.25, "chest", $"P_spectre_dmg_fire", false )
	ModelFX_EndData()

	ModelFX_BeginData( "damagefx", model_name, "all", true )
	  	ModelFX_AddTagBreakGib( 1, "left_leg", leftLegGib, GIBTYPE_NORMAL, 100, 200 )
			ModelFX_AddTagBreakFX( 1, "FX_L_LEG", $"P_sparks_dir_MD_LOOP", "" )

	  	ModelFX_AddTagBreakGib( 1, "right_leg", rightLegGib, GIBTYPE_NORMAL, 100, 200 )
			ModelFX_AddTagBreakFX( 1, "FX_R_LEG", $"P_sparks_dir_MD_LOOP", "" )

	  	ModelFX_AddTagBreakGib( 1, "left_arm", leftArmGib, GIBTYPE_NORMAL, 100, 200 )
			ModelFX_AddTagBreakFX( 1, "FX_L_ARM", $"P_sparks_dir_MD_LOOP", "" )

	  	ModelFX_AddTagBreakGib( 1, "right_arm", rightArmGib, GIBTYPE_NORMAL, 100, 200 )
			ModelFX_AddTagBreakFX( 1, "FX_R_ARM", $"P_sparks_dir_MD_LOOP", "" )
	ModelFX_EndData()
